...
(function($) {
    ...
    $(document).ready( function() {
        ...
        var slickRtl = $('html').attr('dir') === 'rtl';
    ...
    function initClientsMarquee() {
        var $track = $('.clients-marquee-track');
        if (!$track.length || $track.data('marquee-ready')) {
            return;
        }

        var $baseSet = $track.children('.clients-marquee-set').first();
        if (!$baseSet.length) {
            return;
        }
        ...
        function buildMarquee() {
            ...
            $baseSet = $track.children('.clients-marquee-set').first();
            $track.children('.clients-marquee-set').not($baseSet).remove();

            while ($track[0].scrollWidth < window.innerWidth * 2) {
                $baseSet.clone().attr('aria-hidden', 'true').appendTo($track);
            }

            var loopWidth = $track[0].scrollWidth;
            $track.children('.clients-marquee-set').clone().attr('aria-hidden', 'true').appendTo($track);

            $track.css('--marquee-distance', (-loopWidth) + 'px');
            $track.addClass('is-ready').data('marquee-ready', true);
        }

        buildMarquee();

        var resizeTimer;
        $(window).off('resize.clientsMarquee').on('resize.clientsMarquee', function() {
            clearTimeout(resizeTimer);
            resizeTimer = setTimeout(function() {
                $track.removeData('marquee-ready').removeClass('is-ready');
                buildMarquee();
            }, 200);
        });
    }
    ...
    function initTestimonialsSlider() {
        var $slider = $('.customer-feedback-carousel-active');
        if (!$slider.length) {
            return;
        }
    ...
    window.PST_rebuildClientsMarquee = function() {
        var $track = $('.clients-marquee-track');
        if (!$track.length) {
            return;
        }
        $track.removeData('marquee-ready').removeClass('is-ready');
        $track.children('.clients-marquee-set').not(':first').remove();
        initClientsMarquee();
    };

    window.PST_rebuildTestimonials = function() {
